...
SERVICE_URL=$(gcloud run deploy "${SERVICE_NAME}" \
  ...
  --no-cpu-throttling  \
  --project "${PROJECT_ID}" \
  --set-env-vars "SLACK_BOT_TOKEN=${SLACK_BOT_TOKEN},SLACK_SIGNING_SECRET=${SLACK_SIGNING_SECRET},GOOGLE_CLOUD_PROJECT=${PROJECT_ID},GOOGLE_CLOUD_LOCATION=${REGION},ALLOWED_SLACK_WORKSPACE=${ALLOWED_SLACK_WORKSPACE:-}" \
