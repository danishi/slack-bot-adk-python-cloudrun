...
SERVICE_URL=$(gcloud run deploy "${SERVICE_NAME}" \
  ...
  --no-cpu-throttling  \
  --project "${PROJECT_ID}" \
  --set-env-vars "^@^SLACK_BOT_TOKEN=${SLACK_BOT_TOKEN}@SLACK_SIGNING_SECRET=${SLACK_SIGNING_SECRET}@GOOGLE_GENAI_USE_VERTEXAI=${GOOGLE_GENAI_USE_VERTEXAI}@GOOGLE_CLOUD_PROJECT=${PROJECT_ID}@GOOGLE_CLOUD_LOCATION=global@ALLOWED_SLACK_WORKSPACE=${ALLOWED_SLACK_WORKSPACE:-}@ALLOWED_SLACK_USERS=${ALLOWED_SLACK_USERS:-}@ALLOWED_SLACK_BOTS=${ALLOWED_SLACK_BOTS:-}@ACCESS_DENIED_MESSAGE=${ACCESS_DENIED_MESSAGE:-}@MODEL_NAME=${MODEL_NAME:-gemini-3.5-flash}@IMAGE_MODEL_NAME=${IMAGE_MODEL_NAME:-gemini-3.1-flash-image}@REACTION_PROCESSING=${REACTION_PROCESSING:-}@REACTION_COMPLETED=${REACTION_COMPLETED:-}@REACTION_TRIGGER=${REACTION_TRIGGER:-robot_face}" \
